...
generate_serial_sets () {
    ...
    /usr/local/bin/macserial \
        ...
        --model "${DEVICE_MODEL}" \
        | while IFS='\ \|\ ' read -r DEVICE_SERIAL BOARD_SERIAL; do
            # make a uuid...
...
"${DEVICE_MODEL}","${DEVICE_SERIAL}","${BOARD_SERIAL}","${UUID}","${MAC_ADDRESS}","${WIDTH}","${HEIGHT}","${KERNEL_ARGS}"
EOF
...
${DEVICE_MODEL}${T}${DEVICE_SERIAL}${T}${BOARD_SERIAL}${T}${UUID}${T}${MAC_ADDRESS}${T}${WIDTH}${T}${HEIGHT}${T}${KERNEL_ARGS}
EOF
            ...
            if [ "${CREATE_ENVS}" ] || [ "${CREATE_PLISTS}" ] || [ "${CREATE_BOOTDISKS}" ] || [ "${OUTPUT_BOOTDISK}" ] || [ "${OUTPUT_ENV}" ]; then
                OUTPUT_ENV_FILE="${OUTPUT_ENV:-"${OUTPUT_DIRECTORY}/env.sh"}"
...
EOF
            ...
            if [ "${CREATE_PLISTS}" ] || [ "${CREATE_BOOTDISKS}" ]; then
                ...
                sed -e s/\{\{DEVICE_MODEL\}\}/"${DEVICE_MODEL}"/g \
                    ...
                    "${MASTER_PLIST}" > "${OUTPUT_DIRECTORY}/plists/${DEVICE_SERIAL}.config.plist" || exit 1
            fi
            ...
            if [ "${CREATE_BOOTDISKS}" ] || [ "${OUTPUT_BOOTDISK}" ]; then
                ...
                ./opencore-image-ng.sh \
                    --cfg "${OUTPUT_DIRECTORY}/plists/${DEVICE_SERIAL}.config.plist" \
                    --img "${OUTPUT_BOOTDISK:-${OUTPUT_DIRECTORY}/bootdisks/${DEVICE_SERIAL}.OpenCore-nopicker.qcow2}" || exit 1
            fi
